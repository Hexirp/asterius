...
echo 'deb [check-valid-until=no] http://snapshot.debian.org/archive/debian/20200721T084125Z stretch main contrib non-free' > /etc/apt/sources.list
echo 'deb [check-valid-until=no] http://snapshot.debian.org/archive/debian-security/20200721T084125Z stretch/updates main contrib non-free' >> /etc/apt/sources.list
echo 'deb [check-valid-until=no] http://snapshot.debian.org/archive/debian/20200721T084125Z stretch-updates main contrib non-free' >> /etc/apt/sources.list
apt update
apt full-upgrade -y
apt install -y \
  automake \
